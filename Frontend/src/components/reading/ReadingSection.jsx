...
function ReadingSection() {
  ...
  const handleBooksClick = () => {
    setShowBooks(true);
    setShowStories(false); // Hide Stories when Books is shown
  };

  const handleStoriesClick = () => {
    setShowStories(true);
    setShowBooks(false); // Hide Books when Stories is shown
  };
    ...
    <div className="flex h-screen">
      <div className="flex-1 flex flex-col p-10 bg-white">
        ...
        <h3 className="text-xl font-bold italic mb-6">
          Choose what you read today!
        </h3>

        {/* If neither Books nor Stories is clicked, show options */}
        <AnimatePresence>
          {!showBooks && !showStories && (
            <motion.div
        ...
        {/* Show BooksSection if Books button is clicked */}
        <AnimatePresence>
          {showBooks && (
            <motion.div
              initial="hidden"
        ...
        {/* Show Stories if Stories button is clicked */}
        <AnimatePresence>
          {showStories && (
            <motion.div
              initial="hidden"
